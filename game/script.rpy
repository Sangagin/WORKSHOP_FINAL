...
label start:
    ...
    # Decision session studio de la journée
        menu:
            ...
            "J'ai envie de voir ce que peut donner le groupe de \"Perruquiers noirs\" aujourd'hui.":
                $amitie_punk += 1
                jump choix3_punk
     #       "Je pourrais offir un de mes objets à quelqu'un":
     #           jump kdo


    label kdo:
        "A qui je veux ouvrir un cadeau ?"
        menu:
            "Un bracelet à Marie-Anne":
                jump kdoGoth
            "Un pendentif à Jeanne":
                jump kdoHippie
            "Une canette de bière à Patrick":
                jump kdoPunk
            "J'ai changé d'avis":
                jump finoffre2

    label kdoGoth:
        if "bracelet" in list(inventaire):
            show gotgift03  
            "Marie-Anne""Oh, merci beaucoup !"
            $amitie_got+=1
        else:
            "Je n'ai pas d'objet à lui offrir."
    ...
    label kdoHippie:
        if pendentif in list(inventaire):
            show  romanecollier2
            $ inventaire.remove("pendentif")
            "Jeanne""Ouah, il est magnifique, c'est trop gentil !"
        ...
        if jour == 5:
            jump retourkdo5
    label kdoPunk:
        if "canette" in list(inventaire):
            show hugobeer 
            "Patrick""C'est de la bonne, merci à toi !"
            $amitie_punk+=1
        else:
            "Je n'ai pas d'objet à lui offrir."
        hide hugobeer
